...
RET_CODE=$(curl $URL -s -o $RESULT -w "%{http_code}")
 ...
 if [ $RET_CODE -eq 200 ];then
	 ...
	 echo "Modtile OK:|NoResp200=$NoResp200, NoResp304=$NoResp304, NoResp404:$NoResp404, NoResp503=$NoResp503, NoFreshCache=$NoFreshCache, NoOldCache=$NoOldCache, NoVeryOldCache=$NoVeryOldCache, NoFreshRender=$NoFreshRender, NoOldRender=$NoOldRender, NoTileBufferReads=$NoTileBufferReads, DurationTileBufferReads=$DurationTileBufferReads, NoRes200Layer=$NoRes200Layer, NoRes404Layer=$NoRes404Layer" 
         exit 0
 else
