...
def print_message_exit(reported, truncate, limit):
...
def rabbit_queues(command='sudo /usr/sbin/rabbitmqctl list_queues',
    ...
    try:
        ...
        if debug:
            for i in queues:
                print(i)
        queues = set(q for q, d in queues if int(d) >= limit)
        if not no_double:
            persist = Persistence()
            try:
                offenders = queues.intersection(persist.read().split('\n'))
                if offenders:
                    print_message_exit(offenders, truncate, limit)
            finally:
                persist.write('\n'.join(queues))
        else:
            if queues:
                print_message_exit(queues, truncate, limit)
